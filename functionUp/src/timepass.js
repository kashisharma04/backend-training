...
const hardCover= async function (req, res) {
 ...
 let books=null;
    for(i of pname){
        books = await bookModel.updateMany({publisher_id:i._id},{$set:{isHardCover:'true'}},{new:true})
    }
...
// let new = await bookModel.updateMany({$0r:[{price:300},{price:45}],
    // {$set :[{name:"Kashi"}]},{new:true}})

// let me = await bookModel.updateOne({price:{$in:[10,34]}},{upsert:true})